...
1. Read `PACKAGE_ROOT/package.json` for companion source names in `piSetup.requiredPackages`, package resources, and setup metadata; read the companion table in `README.md` for minimum versions.
...
- **Companion package-owned:** the separate Pi package sources named by `piSetup.requiredPackages`, subject to documented minimum versions.
...
- Presence and installed source/version of every separately managed package in `piSetup.requiredPackages`; installed versions at or above documented minimums are compliant, not drifted.
...
- `drifted`: present below documented minimum, from a different source, or otherwise differs from intended approved state. Newer versions satisfying minimums are compliant.
...
8. Keep companion packages as separate Pi-managed sources. Install or update only an explicitly approved source; use unversioned npm source for new installs, and accept any installed version at or above the documented minimum. Never remove one as a first-party-package duplicate.
