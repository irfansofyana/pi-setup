...
| `reviewer` | Independent diff, risk, and test review | Read-only; no shell or extensions | `code-review` |
...
5. the parent runs real verification and supplies the actual diff plus results to reviewer for independent, read-only assessment.
...
4. Ask `reviewer` to assess the supplied diff, file context, and verification evidence; it cannot run commands itself.
