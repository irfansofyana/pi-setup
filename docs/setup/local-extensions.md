...
TUI opens a theme-aware scrollable overlay; `q`, `Escape`, or `Ctrl-C` closes it. Non-TUI modes print readable text. The command does not call models, append messages, mutate session/config, or write cache. Run `/reload` after package updates.
